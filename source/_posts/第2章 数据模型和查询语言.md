...
[^nosql]: 作者说着说着文档模型就讲到NoSQL上了，我个人也觉得这两者即使不等价，也是差不离。
...
其实json并非没有模式，其实一个是写式模式（显式），一个读时模式（隐式），格式易修改，不用像关系数据库得修改表结构。

json拥有更好的局部性，不用联接（join）多张表或者多次查询，但仅限于访问大部分数据情况，无法像关系数据库返回特定字段。

json擅长处理1 vs N，不擅长处理N vs 1和N vs N，只能依赖在应用层使用代码做，另外是不是还增加了数据冗余度，因为不能像关系数据库那样通过外键引用同一份数据。
...
在可预见的未来，大多数系统同时使用关系数据库和NoSQL数据库，称为混合持久化。
...
一个来自社交网络的族谱数据库，雷车爱达荷州的Lucy和来自法国波恩的Alain，他们结婚了，目前住在伦敦。
...
如果是关系数据表实现，相应地可以用SQL查询，但是使用SQL遍历图比较麻烦。
...
也就是Turtle是给人看的三元组，RDF是给人看的三元组。

> 语义网
>
> 语义网源自大家想创建一种囊括互联网所有数据的设想，为了创建这种数据库，需要所有人都遵守一种通用格式，就像人为了能够理解彼此，必须得说同样地语言，为了描述这种格式诞生了RDF。

总结下，尽管上面关系模p型、文档模型、网络模型、图模型四种模型都叫“模型”，但是并不是同一维度的概念，前两者是实现方式，后两者是表现形式，比如图模型可以使用专门的图数据库实现，也可以用关系模型模拟。
...
[^sparql]: 简不简洁仁者见仁，但是看起来更像SQL倒是真的。
